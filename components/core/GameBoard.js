...
const GameBoard = () => {
    ...
    const { participants, playersAreReady, game, channel } = useGameContext()
    ...
    if (game?.status === GAME_STATE.PLAYING) {
        const player = participants?.playerOne?.id === user?.id ? PLAYER_DETAILS.PLAYER_ONE : PLAYER_DETAILS.PLAYER_TWO
        console.log(player, 'this is palyer')
        body = (
